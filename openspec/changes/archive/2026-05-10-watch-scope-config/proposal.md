...
The watcher infrastructure to do this already exists. [`SolutionWatcher`](src/DevBitsLab.Mcp.SourceGraph.Watcher/SolutionWatcher.cs) handles debounced `FileSystemWatcher` events with parse-tolerant batching, and [`LiveIndexService`](src/DevBitsLab.Mcp.SourceGraph.Server/LiveIndexService.cs) already owns a per-scope add/index/teardown lifecycle (`PrepareScopeAsync` → `RunInitialIndexAsync` → `StartWatcher` → `DisposeAsync`). What's missing is a watcher pointed at `.sourcegraph.json` plus a diff-and-apply step that drives the existing lifecycle in response.
...
- **Memory / runtime**: One additional `FileSystemWatcher` handle (negligible). No memory growth proportional to scope count beyond what already happens at startup.
