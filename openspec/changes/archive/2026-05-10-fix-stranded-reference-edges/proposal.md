...
- **Pass 1's "unchanged file" skip path gains a references-intact integrity check.** Today the check is `unchanged && !fullReset && _keysByFileId.ContainsKey(fileId)` — match the SHA, in-memory map populated, skip both passes. After this change, the skip ALSO requires evidence that the file's outgoing references are present in the store: a non-zero count of `refs` rows whose `file_id = ?`, when the file declares any symbols. Files that declare symbols but have zero outgoing rows fall through to pass 2 for re-walking.
- **A new `IGraphStore.HasOutgoingReferencesAsync(long fileId, CancellationToken ct)` storage method** returns true if at least one `refs` row exists for the given file. Cheap query (`SELECT EXISTS (SELECT 1 FROM refs WHERE file_id = ? LIMIT 1)`); fires once per unchanged file at index startup, runs against an indexed column.
