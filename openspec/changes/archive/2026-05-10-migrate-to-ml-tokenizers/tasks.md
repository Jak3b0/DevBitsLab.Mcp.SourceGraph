...
- [x] 2.3 If not permitted, add a one-time fetch step in the unit test class's `IClassFixture` setup that downloads the file into `tests/fixtures/.cache/` (gitignored) on first run; subsequent runs use the cached copy. Document the choice in the test file's class summary. *(Implemented in `JinaTokenizerLoadTests.TokenizerFixture`: prefers the live `~/.cache/devbitslab.sourcegraph/...` copy if present, then a per-project gitignored cache, then HTTP download from HF. Sets `SkipReason` and the test no-ops on a network-restricted box.)*
